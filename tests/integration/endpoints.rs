...
mod openai_responses_endpoint {
	...
	fn create_comprehensive_request() -> OpenAIResponsesRequestPayload {
		...
		OpenAIResponsesRequestPayload {
			model: Some("openai/gpt-4o-mini".to_string()),
			input: Some(OpenAIInputMessage::Items(vec![ResponseInputItem::Message(InputMessage {
				role: InputMessageRole::User,
				content: InputMessageContent::Parts(vec![ResponseInputContentPart::InputText(ResponseInputText {
	...
	async fn test_tool_usage_request() {
		...
		let request_payload = OpenAIResponsesRequestPayload {
			model: Some("openai/gpt-4o-mini".to_string()),
			input: Some(OpenAIInputMessage::String("What's the weather in San Francisco?".to_string())),
...
mod ollama_endpoint {
	...
	async fn test_ollama_model_discovery() {
		...
		if server.add_provider(common::create_ollama_endpoint()).await.is_ok() {
