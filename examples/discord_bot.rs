...
use omniference::{
	OmniferenceEngine,
	types::{ChatRequestIR, Message as OmniMessage, ModelRef, ProviderConfig, ProviderEndpoint, ProviderKind},
};
...
async fn main() -> anyhow::Result<()> {
	...
	let mut client = Client::builder(&token, intents).event_handler(Handler { engine: Arc::new(engine) }).await?;
