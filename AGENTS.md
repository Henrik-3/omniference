...
- `GeminiAdapter` - Google Gemini Interactions API; native Gemini images also use Interactions, while Imagen uses its dedicated predict endpoint
...
- **Gemini**: Use the stable, stateless `/v1/interactions` API with `store = false`; preserve provider tool-call IDs and keep Imagen on its dedicated `predict` endpoint.
