...
impl TestConfig {
    ...
    pub fn load_from_env() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if let Ok(api_key) = std::env::var("OPENAI_API_KEY") {
            ...
            if let Some(openai_compat_provider) = config
                .providers
                .iter_mut()
                .find(|p| p.name == "openai_compat")
            {
    ...
    pub fn should_skip_live_tests(&self) -> bool {
        self.test_settings.skip_live_tests
            || std::env::var("SKIP_LIVE_TESTS").map_or(false, |v| v.to_lowercase() == "true")
    }

    pub fn log_responses(&self) -> bool {
        self.test_settings.log_responses
            || std::env::var("LOG_RESPONSES").map_or(false, |v| v.to_lowercase() == "true")
    }
}
