...
impl ChatAdapter for OpenAIResponsesAdapter {
    ...
    async fn execute_chat(
        ...
        if ir.stream {
            ...
            let response: OpenAIResponsesResponse = resp.json().await
...
impl OpenAIResponsesAdapter {
    fn build_openai_request(ir: &ChatRequestIR) -> Result<OpenAIResponsesRequest, AdapterError> {
        let input: Vec<OpenAIInputMessage> = ir.messages.iter()
            .map(|msg| {
                let content_parts: Vec<OpenAIContentPart> = msg.parts.iter()
                    .map(|part| {
                        match part {
                            ContentPart::Text(text) => OpenAIContentPart::InputText { text: text.clone() },
                ...
                match msg.role {
                    Role::System => OpenAIInputMessage::SystemMessage { content: content_parts },
                    Role::User => OpenAIInputMessage::UserMessage { content: content_parts },
                    Role::Assistant => OpenAIInputMessage::AssistantMessage { content: content_parts },
        ...
        let reasoning_effort = ir.metadata.get("reasoning_effort")
            .cloned()
            .unwrap_or_else(|| "medium".to_string());
        
        let reasoning_summary = ir.metadata.get("reasoning_summary")
            .cloned();
        
        let verbosity = ir.metadata.get("text_verbosity")
            .cloned();
